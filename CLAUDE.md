...
agentmux is a CLI tool for managing parallel AI coding sessions using git worktrees, tmux, and AI agents. It lets developers run multiple isolated coding tasks in parallel, each with its own branch, worktree, and terminal pane.
...
**Libraries** (`src/lib/`):
- `config.ts` — Reads/writes `~/.agentmux/config.json`, merges defaults
- `sessions.ts` — CRUD for `~/.agentmux/sessions.json` (tracks active sessions with id, repo, branch, tmux pane, agent, status)
...
- `context.ts` — Generates `AGENTMUX.md` files in each worktree so agents know about sibling sessions
